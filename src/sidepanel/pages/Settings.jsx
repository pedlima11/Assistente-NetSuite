...
export default function Settings() {
  ...
  async function handleTestConnection() {
    ...
    try {
      const [general, fte] = await Promise.all([
        sendToBackground({ type: 'TAX_DISCOVERY', action: 'taxDiscovery' }),
        sendToBackground({ type: 'TAX_DISCOVERY', action: 'taxDiscoveryFTE' }),
      ]);
      if (general.error) throw new Error(general.error);
      if (fte.error) throw new Error(fte.error);
      const merged = { ...general, ...fte, _summary: { ...general._summary, ...fte._summary } };
      setDiscoveryResult(merged);
    ...
    <div className="max-w-4xl">
      ...
      <div className="mt-4 bg-white rounded-lg border border-ocean-30 p-4 space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-ocean-150 text-white rounded-md text-xs font-medium hover:bg-ocean-180 disabled:opacity-50 disabled:cursor-not-allowed"
        >
